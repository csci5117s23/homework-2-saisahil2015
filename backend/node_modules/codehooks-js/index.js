...
const debug = Debug("codehooks-js");
import cronjob from "./cronjob.mjs"
import coderun from './coderunner.mjs'
 ...
 * @property {function(collection.<string>):NoSQLAPI} getCollection - Get database collection by string name, returns an NoSQL API object
 ...
 * @property {function("data"|"end"|"error", callback.<Function>):DataStream} on - Emits data, stream.on('data', (data) => console.log(data))
 ...
 */
 ...
 class Codehooks {
   ...
   *  console.log("Global middleware was here!");
   *  next(); // proceed
   * })
   * // middleware on a specific route
   * app.use('/myroute', (req, res, next) => {
   *  console.log("Route middleware was here!");
   *  next(); // proceed
   * })
 ...
 */
  ...
  */
  useExpress = async (express, options) => {
    ...
    // apply middleware
    debug('middleware', this.appmiddleware)
    for (let key in this.appmiddleware) { 
      const func = this.appmiddleware[key];     
      debug('Apply express middleware', func);
      if (func.path) {
    ...
    // apply routes
    debug('routes', this.routes)
    for (let key in this.routes) {
      const func = this.routes[key];
      const [method, route] = key.split(' ');
      debug('Apply express routes', method, route, ...func);
      switch (method) {
